...
const QualifiactionDetails = ({ formData, setFormData, prevData, onButtonClick, view }) => {
  ...
  const fetchData = () => {
    axios.get(`http://141.148.194.18:8052/payroll/employee/educational-details/${formData.empRefNo}`, {
      ...
      const mappedRows = result.map((item, index) => ({
        id: index + 1,
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Button onClick={() => {
            const deleteObj = {
              qualificationId: params.row.qualificationId,
              instituteName: params.row.instituteName,
              boardOrUniversity: params.row.boardOrUniversity,
              course: params.row.course,
              marksCgpaId: params.row.marksCgpaId,
              marksSecured: params.row.marksSecured,
              totalMarks: params.row.totalMarks,
              cgpa: params.row.cgpa,
              admissionDate: params.row.admissionDate,
              completionDate: params.row.completionDate
            };
            AlertConfirm({
              ...
              onOk: () => {
                ...
                setFormData({
                  ...formData,
                  educationalDetails: updatedRows
                });
              },
              onCancel: () => {
                console.log("cancel");
    ...
    <form>
      <Box pt={2} pb={4}>
        <Card elevation={3} sx={{ p: 2 }}>
          <H3 mb={2}>Educational Details</H3>
          <Divider />
          <CardContent>
            {view ? (
              <SearchTable
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <Autocomplete
                    ...
                    onChange={(event, value) => {
                      formik.setFieldValue("QualifiactionTypes", value?.id || null);
                    }}
                    value={getValueFromList(qualificationList, formik.values.QualifiactionTypes)}
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label="Qualification Type"
                        variant="outlined"
                        error={Boolean(formik.touched.QualifiactionTypes && formik.errors.QualifiactionTypes)}
                ...
                <Grid item xs={12} md={6}>
                  <Autocomplete
                    ...
                    getOptionLabel={(option) => option.label}
                    onChange={(event, value) => {
                      formik.setFieldValue("degree", value?.id || null);
                    }}
                    value={getValueFromList(degreeList, formik.values.degree)}
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label="Degree"
                ...
                <Grid item xs={12} md={6}>
                  <Autocomplete
                    ...
                    getOptionLabel={(option) => option.label}
                    onChange={(event, value) => {
                      formik.setFieldValue("marksCgpa", value?.id || null);
                    }}
                    value={getValueFromList(marksCgpaList, formik.values.marksCgpa)}
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label="Marks/Cgpa"
                        variant="outlined"
                        error={Boolean(formik.touched.marksCgpa && formik.errors.marksCgpa)}
                ...
                <Grid item xs={12} md={6}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DatePicker
                      ...
                      onChange={(value) => {
                        formik.setFieldValue("admissionDate", value ? value.format('YYYY-MM-DD') : null);
                      }}
                      renderInput={(params) => (
                        <TextField
                          {...params}
                ...
                <Grid item xs={12} md={6}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DatePicker
                      ...
                      onChange={(value) => {
                        formik.setFieldValue("completionDate", value ? value.format('YYYY-MM-DD') : null);
                      }}
                      renderInput={(params) => (
                        <TextField
                          {...params}
